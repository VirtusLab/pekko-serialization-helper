  ...
  override def newPhase(prev: Phase): Phase =
    new StdPhase(prev) {
      ...
      private val genericsToKinds: Map[String, Seq[ClassKind]] = Map(
        "org.apache.pekko.actor.typed.ActorSystem" -> Seq(ClassKind.Message),
        "org.apache.pekko.actor.typed.ActorRef" -> Seq(ClassKind.Message),
        "org.apache.pekko.actor.typed.Behavior" -> Seq(ClassKind.Message),
        "org.apache.pekko.actor.typed.RecipientRef" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.PipeToSupport.PipeableFuture" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.PipeToSupport.PipeableCompletionStage" -> Seq(ClassKind.Message),
        "org.apache.pekko.persistence.typed.scaladsl.ReplyEffect" -> Seq(ClassKind.PersistentEvent, ClassKind.PersistentState),
        "org.apache.pekko.persistence.typed.scaladsl.Effect" -> Seq(ClassKind.PersistentEvent),
        "org.apache.pekko.persistence.typed.scaladsl.EffectBuilder" -> Seq(ClassKind.PersistentEvent),
        "org.apache.pekko.projection.eventsourced.EventEnvelope" -> Seq(ClassKind.PersistentEvent, ClassKind.PersistentState))

      private val genericMethodsToKinds: Map[String, Seq[ClassKind]] = Map(
        "org.apache.pekko.actor.typed.scaladsl.ActorContext.ask" -> Seq(ClassKind.Message, ClassKind.Message),
        "org.apache.pekko.actor.typed.scaladsl.AskPattern.Askable.$qmark" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.PipeToSupport.pipe" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.PipeToSupport.pipeCompletionStage" -> Seq(ClassKind.Message))

      private val concreteMethodsToKinds: Map[String, Seq[ClassKind]] = Map(
        "org.apache.pekko.actor.typed.ActorRef.ActorRefOps.$bang" -> Seq(ClassKind.Message),
        "org.apache.pekko.actor.typed.ActorRef.tell" -> Seq(ClassKind.Message),
        "org.apache.pekko.actor.typed.RecipientRef.tell" -> Seq(ClassKind.Message))

      private val concreteUntypedMethodsToKinds: Map[String, Seq[ClassKind]] = Map(
        "org.apache.pekko.actor.ActorRef.tell" -> Seq(ClassKind.Message, ClassKind.Ignore),
        "org.apache.pekko.actor.ActorRef.$bang" -> Seq(ClassKind.Message),
        "org.apache.pekko.actor.ActorRef.forward" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.AskSupport.ask" -> Seq(ClassKind.Ignore, ClassKind.Message, ClassKind.Ignore),
        "org.apache.pekko.pattern.AskSupport.askWithStatus" -> Seq(ClassKind.Ignore, ClassKind.Message, ClassKind.Ignore),
        "org.apache.pekko.pattern.AskableActorRef.ask" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.AskableActorRef.askWithStatus" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.AskableActorRef.$qmark" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.AskableActorSelection.ask" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.AskableActorSelection.$qmark" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.ExplicitAskSupport.ask" -> Seq(ClassKind.Ignore, ClassKind.Message, ClassKind.Ignore))

      private val concreteHigherOrderFunctionsToKinds: Map[String, Seq[ClassKind]] = Map(
        "org.apache.pekko.pattern.ExplicitlyAskableActorRef.ask" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.ExplicitlyAskableActorRef.$qmark" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.ExplicitlyAskableActorSelection.ask" -> Seq(ClassKind.Message),
        "org.apache.pekko.pattern.ExplicitlyAskableActorSelection.$qmark" -> Seq(ClassKind.Message))
              ...
              case None =>
                reporter.error(
                  ...
                  s"""${typeWithClassType.typ
                      .toString()} is used as Pekko ${typeWithClassType.classKind.name} but does not extend a trait annotated with $serializabilityTraitType.
